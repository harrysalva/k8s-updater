    ...
    <Box sx={{ p: 3, maxWidth: 960 }}>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {running ? 'Running checks…' : 'Run pre-upgrade scan (15 checkers)'}
          </Button>
          {report && !running && (
        ...
        <>
            ...
            <Box sx={{ ml: 'auto' }}>
              {report.blocker ? (
                ...
                <Alert severity="success" sx={{ py: 0.25, px: 1.5 }}>
                  <strong>No blockers detected</strong> — by the 15 checkers in this scan
                </Alert>
              )}
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
            <Tooltip title="This scan covers 14 deterministic checks. It does not guarantee a safe upgrade — additional risks may exist outside its scope. See docs/COVERAGE.md for the full list of what is and is not detected.">
              <Chip
                label="Coverage: 15 checkers active"
                size="small"
                variant="outlined"
                color="default"
